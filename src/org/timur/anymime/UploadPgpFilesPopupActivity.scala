...
class UploadPgpFilesPopupActivity extends Activity {
  ...
  private var sshPassphraseEditView:EditText = null
  ...
  private var sshPassphrase:String = null
  ...
  private def upload() {
    ...
    if(sshPassphraseEditView!=null)
      sshPassphrase = sshPassphraseEditView.getText.toString
    ...
    new Thread() {
      override def run() {
        try {
          SshHelper.scp(sshHostname, sshUsername, sshLocalKeyPath, srcFilePathNameString, sshTargetPath, sshTargetFilename, sshPassphrase)
  ...
  private def setPgpUploadDialog() {
    ...
    sshPassphraseEditView = findViewById(R.id.sshPassphrase).asInstanceOf[EditText]
