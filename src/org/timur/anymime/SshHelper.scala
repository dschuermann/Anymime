...
object SshHelper {
  ...
  def scp(hostname:String, username:String, publicKeyPath:String, sendFilePath:String, remoteDirectory:String, remoteFileName:String, passphrase:String) :Unit = {
    // note: the caller has to prompt exception messages to the user

    if(D) Log.i(TAG, "scp username="+username+" hostname="+hostname+" publicKeyPath="+publicKeyPath+" sendFilePath="+sendFilePath+" remoteFileName="+remoteFileName+" passphrase="+passphrase)
    JSch.setLogger(new MyLogger)
    val jsch = new JSch
    try {
      if(passphrase!=null && passphrase.length>0)
        jsch.addIdentity(publicKeyPath,passphrase)
      else
